...
def main():
    ...
    for entry in data:
        ...
        # Pad or truncate trace to INPUT_SIZE
        if len(trace) < INPUT_SIZE:
            trace = np.pad(trace, (0, INPUT_SIZE - len(trace)), 'constant')
        elif len(trace) > INPUT_SIZE:
            trace = trace[:INPUT_SIZE]
        traces.append(trace)
        labels.append(entry['website_index'])  # Use website_index from dataset

    traces = np.array(traces, dtype=np.float32)
    labels = np.array(labels, dtype=np.int64)

    # Normalize the traces using Min-Max normalization
    min_val = np.min(traces)
    max_val = np.max(traces)
    traces = (traces - min_val) / (max_val - min_val) if max_val != min_val else traces  # Avoid division by zero

    # Derive num_classes from dataset
    num_classes = len(np.unique(labels))
    print(f"Derived num_classes from dataset: {num_classes}")

    # # Validate and adjust labels to ensure they are 0-based and sequential
    # if not all(0 <= label < num_classes for label in labels):
    #     print("Warning: Label values are out of range [0, {}]. Adjusting labels...".format(num_classes - 1))
    #     label_map = {old: new for new, old in enumerate(sorted(np.unique(labels)))}
    ...
    for model_name, model in models.items():
        ...
        preds, true_labels = evaluate(model, test_loader, WEBSITES)
        results[model_name] = {"accuracy": best_accuracy, "preds": preds, "labels": true_labels}

    # 6. Print comparison of results
    print("\nModel Comparison:")
    for model_name, result in results.items():
        print(f"{model_name}: Best Accuracy = {result['accuracy']:.4f}")
